...
import { AIVideoBucket, GenerateVideoFromFramesParams } from "./types/video.type";
import { generateVideoByPolling, validateVideoConfigFields } from "./video.util";

/**
 *
 * @param {GenerateVideoFromFramesParams} data Generate video from frames object
 * @return {Promise<string>} the GCS uri of a video
 * @throws {Error} If configuration is invalid or video generation fails.
 */
export async function generateVideoFromFramesFunction(data: GenerateVideoFromFramesParams) {
 ...
 */
function constructVideoArguments(isVeo31Used: boolean, imageParams: GenerateVideoFromFramesParams) {
 ...
 * @param {GenerateVideoFromFramesParams} imageParams    Generate Video from Frames Request
 * @return {string} video uri
 */
async function interpolateVideo(aiVideo: AIVideoBucket, imageParams: GenerateVideoFromFramesParams) {
  try {
